...
export default function AdminContactReviewsPage() {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const res = await contactAPI.getReviews(
        statusFilter ? (statusFilter as "pending" | "approved" | "rejected") : undefined
      );
      if (res.success) setReviews(res.data || []);
    } catch (err: any) {
      error(err.message || "Failed to load contact reviews");
      setReviews([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleStatus = async (id: string, status: "approved" | "rejected") => {
    setUpdatingId(id);
    try {
      await contactAPI.updateReviewStatus(id, status);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <h1 className="text-3xl font-serif font-medium text-neutral-900">
              Contact Page <span className="italic text-pink-600">Reviews</span>
            </h1>
            <p className="text-neutral-500 text-sm mt-1">Approve or reject reviews submitted from contact page.</p>
          </div>
        </div>
        ...
        {reviews.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-neutral-200">
            ...
            <p className="text-neutral-500 text-sm">Contact page reviews will appear here.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left min-w-[760px]">
                <thead className="bg-neutral-50 border-b border-neutral-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-neutral-500">Review</th>
                ...
                <tbody className="divide-y divide-neutral-100">
                  {reviews.map((r) => (
                    <tr key={r._id} className="hover:bg-neutral-50/50">
                      <td className="px-6 py-4 text-sm font-medium text-neutral-900">{`${r.firstName || ""} ${r.lastName || ""}`.trim() || "—"}</td>
                      <td className="px-6 py-4 text-sm text-neutral-700">{r.email || "—"}</td>
                      <td className="px-6 py-4 text-sm text-neutral-600 max-w-[360px]">
                        <p className="line-clamp-2" title={r.review}>{r.review || "—"}</p>
                      </td>
